...
\title{Software Requirements Specification for \progname : An Advanced Version of Conway's Game of Life} 
\author{\authname}
\date{\today}
...
Conway's Game of Life [ \cite{gardner1970life} ] is the result of applying simple rules of evolution.
...
For example, evolutionary rules can be more permissive by allowing the death or birth of a cell under several conditions.
...
He will also need to know the graphics libraries used for this project.
...
\begin{itemize}
\item The user's sole responsibility is to choose values for the parameters from those proposed by the interface.
\end{itemize}
...
No system constraints are identified for this project.
...
\item A cell : A portion of a display area that contains one or more numbers that represent its value(s).t
...
% #5 description
{
  ...
  $A$ is the amplitude of the curve. $\mu$ and $\sigma$ define the shape of the curve.
}
% #6 Notes
{
None.
...
% #7 Source
{
  \url{https://en.wikipedia.org/wiki/Finite_difference\#Multivariate_finite_differences}
}
% #8 Referenced by
{
...
\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
  ...
  & $F$ is a real number that represents the feed rate and $k$ is a real number that represents the kill rate.
  \\
  \hline
  Sources& \url{https://visualpde.com/nonlinear-physics/gray-scott.html} \\
  \hline
  Ref.\ By & \ref{TM:LO} \ref{TM:NL} \\
  \hline
\end{tabular}
\end{minipage}\\
...
\item[R\refstepcounter{reqnum}\thereqnum \label{R_Inputs}:] The system must allow change of values of input data during simulation
...
\item[NFR\refstepcounter{nfrnum}\thenfrnum \label{NFR_Accuracy}:]
  \textbf{Accuracy} The accuracy of the simulation must be such that no display inaccuracy can be detected by the user.

\item[NFR\refstepcounter{nfrnum}\thenfrnum \label{NFR_Usability}:] \textbf{Usability} All users should be able to use this software without external documentation.
...
\item[NFR\refstepcounter{nfrnum}\thenfrnum \label{NFR_Portability}:]
  \textbf{Portability} This program should run on all operating systems.

\end{itemize}

\subsection{Rationale}

Since the projet don't have scope decision, assumption or constraints, this section is not relevant.
